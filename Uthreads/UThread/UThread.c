...
VOID UtExit () {
	NumberOfThreads -= 1;	
	while (!IsListEmpty(&RunningThread->joinedThreads)) {
		PLIST_ENTRY tlink = RemoveHeadList(&RunningThread->joinedThreads);
		PUTHREAD resumeThread =
...
// Newer thread functions

#ifdef WITH_JOIN

// Join implementation with Active wait
BOOL UtJoin0(HANDLE handle) {
	if (handle == UtSelf()) 
		return FALSE;
	while (UtAlive(handle))
		UtYield();
	return TRUE;
}

// Join implementation with passive wait
BOOL UtJoin(HANDLE handle) {
	PUTHREAD thread;
	if (handle == UtSelf())
		return FALSE;
	thread = (PUTHREAD) handle;
	if (!UtAlive(handle)) return FALSE;
...
#ifdef WITH_JOIN
	InitializeListHead(&Thread->joinedThreads);
#endif
	//
